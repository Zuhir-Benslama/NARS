...
function createPermanentLabel(layer, label, phaseKey) {
    ...
    if (phaseKey === 'areas') return;   // areas use the rotated perimeter label
...
// ─── PHASE NAVIGATION ─────────────────────────────────────────────────────────
// Exported so PhaseBar component can call them.
...
export function setPhase(index) {
    ...
    // Show city center dialog if entering that phase for the first time
    if (PHASES[index].key === 'cityCenter' && store.cityCenterMode === null)
        store.cityCenterDialogVisible = true;
}

// City center dialog actions (called from CityCenterDialog component)
export function cityCenterYes() {
    store.cityCenterDialogVisible = false;
    // User now places the marker — draw tool already active
}
...
function registerDrawEvents() {
    ...
    map.on(L.Draw.Event.CREATED, async (event) => {
        ...
        // Populate modal extras before opening
        await prepareModalExtras(phase, layer);

        // Open modal (awaits user interaction via Promise)
...
// Populates store.modal with the data the FeatureModal component needs
// to render phase-specific fields (road list, main entrance list, etc.)

async function prepareModalExtras(phase, layer) {
    const m = store.modal;

    if (phase.key === 'areas') {
        // Check if main urban area already exists
...
// Called by FeatureModal when the user selects a road (to fetch side + suggested number)
...
// Called by FeatureModal when user selects a main entrance
