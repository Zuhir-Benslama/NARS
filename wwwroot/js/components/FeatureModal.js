...
import { PHASES, AREA_TYPES, DISTRICT_TYPES, ROAD_TYPES, PUBLIC_SPACE_TYPES } from '../phases.js';
import { resolveModal, currentModalLayer }  from '../store.js';
import { fetchRoadSide, computeBisNumber }  from '../map.js';
...
export default defineComponent({
    ...
    setup() {
        ...
        const phase = computed(() => PHASES[m.value.phaseIndex] || null);
        ...
        watch(() => m.value.selectedRoadIdx, async (val) => {
            ...
            const roadOption = m.value.roadOptions[parseInt(val)];
            if (!roadOption) return;
            await fetchRoadSide(roadOption.dbId, parseInt(val));
        });

        // When road side is resolved → sync entrance number input
        watch(() => m.value.entranceNumber, (val) => {
            // The input is directly bound to m.entranceNumber via v-model, no extra sync needed
        });

        // When main entrance selection changes → compute BIS number
        watch(() => m.value.selectedMainIdx, (val) => {
            if (val === '' || val === null) return;
            const option = m.value.mainEntranceOptions[parseInt(val)];
            if (!option) return;
            computeBisNumber(option.dbId);
        });

        // When phase opens and area type changes → auto-fill municipality name
        ...
        function onSave() {
            ...
            if (key === 'areas') {
                ...
                const roadOption = m.value.roadOptions[parseInt(m.value.selectedRoadIdx)];
                result.roadDbId       = roadOption?.dbId;
                result.roadLabel      = roadOption?.label;
                result.side           = m.value.entranceSide;
                result.entranceNumber = m.value.entranceNumber;
            } else if (key === 'secondaryEntrances') {
                const mainOption = m.value.mainEntranceOptions[parseInt(m.value.selectedMainIdx)];
                result.mainEntranceDbId  = mainOption?.dbId;
                result.mainEntranceLabel = mainOption?.label;
                result.bisNumber         = m.value.bisNumber;
        ...
        return {
            m, phase, headerText, sideText, bisStr,
            areaTypeOptions, DISTRICT_TYPES, ROAD_TYPES, PUBLIC_SPACE_TYPES,
            onSave, onCancel, onKeyup,
        };
    },
            ...
            <div class="modal-content">
                ...
                <template v-if="phase?.key === 'mainEntrances'">
                    ...
                    <div v-if="m.selectedRoadIdx !== ''" class="modal-field">
                        <label>
                            Entrance Number
                            <span v-if="sideText" style="font-weight:400;text-transform:none;color:#888;"> — {{ sideText }}</span>
                        </label>
