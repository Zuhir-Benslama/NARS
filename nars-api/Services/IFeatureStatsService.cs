...
public interface IFeatureStatsService
{
    ...
    Task<(List<FeatureResult> features, long totalCount)> LoadAllFeaturesAsync(Guid userId, int skip, int take, CancellationToken ct = default);
    /// <summary>Loads features for a user filtered by layer type with pagination.</summary>
    Task<(List<FeatureResult> features, long totalCount)> LoadByLayerAsync(Guid userId, string layer, int skip, int take, CancellationToken ct = default);
}
