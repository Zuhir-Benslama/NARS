...
describe("CommuneList", () => {
  it("renders commune name and user count", () => {
    const communes = [{ commune_id: 1, commune_name_fr: "Algiers", commune_name_ar: "الجزائر", users: [mockUser()] }]
  ...
  it("shows 'no users' when commune has no users", () => {
    const communes = [{ commune_id: 2, commune_name_fr: "Oran", commune_name_ar: "وهران", users: [] }]
    const wrapper = mount(CommuneList, { props: { communes } })
  ...
  it("renders user stats in table", () => {
    const user = mockUser()
    const communes = [{ commune_id: 1, commune_name_fr: "Test", commune_name_ar: "", users: [user] }]
    const wrapper = mount(CommuneList, { props: { communes } })
  ...
  it("renders totals row when multiple users", () => {
    const users = [mockUser({ user_id: 1, username: "a", areas: 2 }), mockUser({ user_id: 2, username: "b", areas: 3 })]
  ...
  it("does not render totals row when single user", () => {
    const communes = [{ commune_id: 1, commune_name_fr: "Test", commune_name_ar: "", users: [mockUser()] }]
    const wrapper = mount(CommuneList, { props: { communes } })
    expect(wrapper.text()).not.toContain("admin.totals")
  })

  it("isFieldWorker returns true for field_worker", () => {
    const communes = [{ commune_id: 1, commune_name_fr: "Test", commune_name_ar: "", users: [mockUser({ role: "field_worker" })] }]
    const wrapper = mount(CommuneList, { props: { communes } })
