...
\texttt{dairas}
  & \texttt{daira\_id} (int)
  & \texttt{wilaya\_id} $\to$ \texttt{wilayas}
  & \texttt{ix\_dairas\_wilaya\_id} \\
\texttt{communes}
  & \texttt{commune\_id} (int)
  & \texttt{daira\_id} $\to$ \texttt{dairas}
  & \texttt{ix\_communes\_daira\_id} \\
...
\texttt{error\_logs}
  & \texttt{id} (UUID v7)
  & \texttt{user\_id} $\to$ \texttt{users} (nullable)
  & \texttt{ix\_error\_logs\_user\_id}, \texttt{ix\_error\_logs\_level}, \texttt{ix\_error\_logs\_created\_at} \\
\texttt{inspections}
  & \texttt{id} (UUID v7)
  & \texttt{user\_id} $\to$ \texttt{users}
  & \texttt{ix\_inspections\_user\_id}, \texttt{ix\_inspections\_feature\_created} \\
\texttt{ai\_draft\_features}
  & \texttt{id} (UUID v7)
  & \texttt{commune\_id} $\to$ \texttt{communes}, \texttt{reviewed\_by} $\to$ \texttt{users} (nullable)
  & \texttt{ix\_ai\_draft\_status}, \texttt{ix\_ai\_draft\_created\_at} \\
\bottomrule
\end{tabularx}
\end{table}
