...
World Geodetic System 1984 (WGS84)\cite{wgs84}, corresponding to EPSG:4326. PostGIS
...
checks) use the Haversine formula \cite{haversine}. Given two geographic coordinates
...
server-side geography distances use the WGS84 spheroid \cite{postgis}, which introduces
...
(latitude-first), while GeoJSON\cite{geojson_rfc} and PostGIS use longitude-first
...
The MapLibre\cite{maplibre} \texttt{map.project(lngLat)} function provides the continuous
...
User passwords are stored as BCrypt digests \cite{bcrypt}. For a password $\pi$ and
...
JSON Web Tokens are signed with HMAC-SHA256 \cite{jwt_rfc,hmac_rfc}. For header $h$, claims
...
CSRF protection covers server-rendered form submissions (the login page)\cite{owasp_csrf}.
...
The \texttt{auth} and \texttt{api} policies are sliding-window limiters \cite{sliding_window};
...
\begin{table}[H]
\centering
\caption{REST API endpoints (authentication, features)}
...
PUT  & \texttt{/api/user/profile}  & Update own username/\allowbreak{}email/\allowbreak{}password & \texttt{api}  \\
...
\begin{table}[H]
\centering
\caption{REST API endpoints (reference data and administration)}
\begin{tabularx}{\textwidth}{ll>{\raggedright\arraybackslash}Xl}
\toprule
\textbf{Method} & \textbf{Path} & \textbf{Description} & \textbf{Rate policy} \\
\midrule
...
\begin{table}[H]
\centering
\caption{REST API endpoints (field, AI draft, auxiliary)}
\begin{tabularx}{\textwidth}{ll>{\raggedright\arraybackslash}Xl}
\toprule
\textbf{Method} & \textbf{Path} & \textbf{Description} & \textbf{Rate policy} \\
\midrule
...
The use of UUID v7 (time-ordered)\cite{uuid7} as the primary key ensures that
...
\begin{table}[H]
\centering
\caption{Database schema summary (reference data and sessions)}
\label{tab:schema}
...
\begin{table}[H]
\centering
\caption{Database schema summary (feature tables)}
\begin{tabularx}{\textwidth}{ll>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X}
\toprule
\textbf{Table} & \textbf{PK} & \textbf{FK} & \textbf{Notable indices / constraints} \\
\midrule
...
\begin{table}[H]
\centering
\caption{Database schema summary (ledger and draft features)}
\begin{tabularx}{\textwidth}{ll>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X}
\toprule
\textbf{Table} & \textbf{PK} & \textbf{FK} & \textbf{Notable indices / constraints} \\
\midrule
