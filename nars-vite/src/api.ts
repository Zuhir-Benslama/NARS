...
async function handleResponse(response: Response, context: { url: string; method: string }): Promise<Response> {
    ...
    switch (response.status) {
        case 403:
            ...
            break
        case 404:
            error = createNotFoundError(message, { ...context, status: response.status })
            break
        case 409:
            error = createConflictError(message, { ...context, status: response.status, code })
            break
        case 422:
            error = createServerError(`Validation failed: ${message}`, { ...context, status: response.status, code })
            break
        case 500:
        case 502:
        case 503:
        case 504:
            error = createServerError(message, { ...context, status: response.status, code })
            break
        default:
            error = createServerError(message, { ...context, status: response.status, code })
    }
    ...
    const executeRequest = async (): Promise<Response> => {
        ...
        if (method !== 'GET' && method !== 'HEAD' && method !== 'OPTIONS') {
            if (!csrfToken) {
                ...
                debugLog('[API] CSRF token is missing for state-changing request. Proceeding without token.')
            } else {
        ...
        try {
            ...
            if (error instanceof TypeError) {
                const networkError = createNetworkError('Network error. Please check your connection.', context, error)
