...
export function closestOnCirclePerimeter(
    ...
    try {
        ...
        const metersPerPixelX = (0.001 * 111320 * Math.cos((centerLat * Math.PI) / 180)) / (offsetPt.x - centerPx.x)
        ...
        return { x: snapX, y: snapY, lng: snapLL.lng, lat: snapLL.lat, dist: Math.abs(cursorDist - radiusPx) }
    } catch {
        return null
    }
}
