...
import { featuresStore } from './state'
import { toApiSaveShape } from './features'
...
export async function undo(): Promise<void> {
    ...
    try {
        ...
        debugWarn(`[UNDO] Restored "${entry.data.label}" with new DB ID ${newDbId} (old: ${entry.dbId})`)
        ...
        if (phaseKey === 'houseEntrances') {
            ...
            for (const entrance of state.houseEntrances || []) {
                if (entrance.data.mainEntranceDbId === oldDbId) {
                    ...
                    debugLog(`[UNDO] Updated secondary entrance "${entrance.data.label}" → mainEntranceDbId ${newDbId}`)
                }
            }
        }
...
function entryDataToGeometry(
    ...
    if (type === 'marker') {
        return { type: 'Point', coordinates: [data.lng ?? 0, data.lat ?? 0] } as GeoJSON.Point
    }
    // City center: restore as LineString circle ring (simple outline)
    if (type === 'circle' && data.lat != null && data.lng != null && data.radius) {
        const ring = computeCircleRing(data.lat, data.lng, data.radius)
        // Close the ring
    ...
    if (data.coordinates && data.coordinates.length > 0) {
        ...
        const ring = first[0] === last[0] && first[1] === last[1] ? coords : [...coords, [first[0], first[1]]]
