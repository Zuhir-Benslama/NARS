...
import { getActiveSnapPhases, snapPointForEdit, setEditDragActive } from './snapping'
import { disableEditMode, getActiveEditEntry, isEditMode } from './edit-mode'
...
export function registerGeomanEvents(): void {
    ...
    map.on('dblclick', (e: MapLibreMapMouseEvent) => {
        ...
        const vertexFeatures = allFeatures.filter((f) => f.source !== 'features' && f.geometry?.type === 'Point')
    ...
    map.on('gm:remove', async (e: GeomanRemoveEvent) => {
        ...
        try {
            const response = await apiFetch(`/api/delete/${dbId}`, { method: 'DELETE' })
            if (!response.ok) {
            ...
            if (removed) {
                ...
                if (entries) {
                    state[phaseKey as keyof LayerState] = entries.filter((f) => f.dbId !== dbId) as never
                }
                syncCounts()
