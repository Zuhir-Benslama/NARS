...
function nearExisting(from: LatLng): boolean {
    ...
    for (const e of existing) {
        if (e.data.lat && e.data.lng) {
            const dist = distance([from.lng, from.lat], [e.data.lng, e.data.lat], { units: 'meters' })
...
function roadStations(coords: LatLng[]): LatLng[] {
    ...
    for (let i = 0; i < coords.length - 1; i++) {
        const a = coords[i]
        const b = coords[i + 1]
        const segLen = distance([a.lng, a.lat], [b.lng, b.lat], { units: 'meters' })
...
async function addPanelIfMissing(label: string, lat: number, lng: number, color: string): Promise<void> {
