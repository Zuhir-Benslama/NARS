...
export function pointInScatteredArea(lat: number, lng: number): boolean {
    return scatteredPolygons.some(
        ({ outer, holes }) => pointInRing(lat, lng, outer) && !holes.some((h) => pointInRing(lat, lng, h)),
    )
}
...
export async function displayCommuneBoundary(communeId: number, _communeName: string): Promise<void> {
    try {
...
export async function refreshScatteredAreas(): Promise<void> {
    try {
        const data = (await apiFetch('/api/areas/refresh-scattered', { method: 'POST' }).then((r) =>
            r.json(),
        )) as ScatteredRefreshResponse
 ...
 */
export function computeCircleRing(lat: number, lng: number, radiusMeters: number): [number, number][] {
 ...
 */
export function computeCircleRingForEdit(lat: number, lng: number, radiusMeters: number): [number, number][] {
    return _computeCircleRing(lat, lng, radiusMeters, GEOMETRY_CONFIG.editCircleSegments)
}

function _computeCircleRing(lat: number, lng: number, radiusMeters: number, segments: number): [number, number][] {
 ...
 */
export function computeCircleRadius(centerLat: number, centerLng: number, ring: [number, number][]): number {
    const total = ring.reduce((sum, [lng, lat]) => sum + haversineDistance(centerLat, centerLng, lat, lng), 0)
 ...
 */
export function makeGeoJsonFeature(geometry: GeoJSON.Geometry, properties: Record<string, unknown>): GeoJSON.Feature {
