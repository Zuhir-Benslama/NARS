...
// Two-phase algorithm ported from nars-vite (Leaflet):
...
//   Phase 3 — DEAD-END CORRECTION: checks graph node degrees to override
//             vote-based orientation for dead-end roads.
...
const toPt = (c: Coord) => turfHelpers.point([c.lng, c.lat])
const toLn = (cs: Coord[]) => turfHelpers.lineString(cs.map((c) => [c.lng, c.lat]))
const dm = (a: Coord, b: Coord) => turfDistance.distance(toPt(a), toPt(b), { units: 'meters' })
...
//   c) Scan body-to-body crossings (X-junctions) via lineIntersect.
//   d) Sort split points along the road, slice into segments.
//   e) Add each segment as an edge in the graph.
//
    ...
    for (const road of roads) {
        ...
        for (const other of roads) {
            ...
            for (const ep of [otherCoords[0], otherCoords[otherCoords.length - 1]]) {
                ...
                const nearest = turfNearest.nearestPointOnLine(line, toPt(ep), { units: 'meters' })
            ...
            for (const feature of intersects.features) {
                ...
                const nearest = turfNearest.nearestPointOnLine(line, toPt(pt), { units: 'meters' })
                junctions.push({
                    segIdx: nearest.properties.index ?? 0,
                    pt,
                })
...
function orientFrom(
    ...
    visitedRoads: Set<string>, // dbIds already fully assigned — cannot be re-assigned
...
export async function computeAndApplyRoadDirections(): Promise<void> {
    ...
    // ── Phase 2: orient ──────────────────────────────────────────────────────
    const cityCenters = (state.cityCenter || []).filter((e) => e.data.lat != null && e.data.lng != null)
    ...
    const votes = new Map<string, { fwd: number; rev: number; entry: LayerEntry; needsReverse?: boolean }>()
    ...
    for (const [_dbId, vote] of votes) {
        ...
        const fromIsFirst = (() => {
            if (degFirst === 1 && degLast === 1) {
                ...
                const ccEntries = (state.cityCenter || []).filter((e) => e.data.lat != null && e.data.lng != null)
    ...
    for (const [dbId, { fwd, rev, entry, needsReverse }] of votes) {
        ...
        if (shouldReverse) {
            ...
            // Update the rendered line on the map immediately
            ...
            try {
                ...
                // Only mutate in-memory data AFTER the API call succeeds
                entry.data.coordinates = reversed
            } catch (err) {
                debugError(`Road direction save error (id=${dbId}):`, err)
                // Rollback the rendered geometry to the original orientation
                featuresStore.update(entry.id, {
                    geometry: {
                        type: 'LineString',
    ...
    // Phase 5: update endpoint markers
 ...
 * Ported from nars-vite addPolylineEndpoints():
...
export function updateEndpointMarkers(): void {
    ...
    for (const road of roads) {
        ...
        const onCityCenter = ccLatLngs.some((cc) => distanceMeters(startLL, cc) < 2)
        if (!onCityCenter) {
            features.push({
                type: 'Feature',
    ...
    // Browser-visible log so the user can verify in console
...
function computeSegmentAngle(a: Coord, b: Coord): number {
    // MapLibre's project() converts lat/lng to pixel coords
 ...
 * Uses Turf.js (already a dependency) for consistency with the rest of this module.
 * For a pure-haversine alternative, see geometry.ts:haversineDistance. */
function distanceMeters(a: Coord, b: Coord): number {
    return turfDistance.distance(toPt(a), toPt(b), { units: 'meters' })
}
