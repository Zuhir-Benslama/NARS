...
import { isNarsError } from '../errors'
...
export function useApiFetch<T = unknown>(): UseApiFetchReturn<T> {
    ...
    async function execute(path: string, options?: ApiFetchOptions): Promise<T | null> {
        ...
        try {
            ...
            if (!response.ok) {
                error.value = new Error(`HTTP ${response.status}`)
                return null
            }
 ...
 * ```
 */
export async function apiRequest<T = unknown>(path: string, options?: ApiFetchOptions): Promise<T | null> {
    try {
        const response = await apiFetch(path, options)
 ...
 * ```
 */
export function isNarsFetchError(err: Error | null): err is import('../errors').NarsError {
