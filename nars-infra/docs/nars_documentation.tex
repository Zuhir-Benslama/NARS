...
codebase; see \texttt{GEOMETRY\_CONFIG.earthRadiusMeters} in
\texttt{src/config.ts}).
...
Urban areas may be of three subtypes: \texttt{central\_urban},
...
\texttt{IBackgroundTaskQueue} (a \texttt{Channel<Func<IServiceProvider,
CancellationToken, ValueTask>>}) so it does not block the HTTP response.
...
\begin{equation}
  c = \{
    ...
    \texttt{email},\;
    \texttt{commune\_id},\;
    \texttt{exp} = t_{\text{now}} + T_{\text{access}}
  \}
  \label{eq:jwt_claims}
\end{equation}

where the access-token lifetime is $T_{\text{access}} = 1{,}440\ \mathrm{min}
= 24\ \mathrm{h}$ (configurable via \texttt{Jwt:ExpiresInMinutes}).
...
exempt because all auth cookies carry \texttt{SameSite=Lax}, which
...
NARS uses ASP.NET Core's sliding-window rate limiter. Two policies are
defined; \cref{tab:rate_limits} gives the default configuration values
from \texttt{appsettings.json}.
...
\midrule
\texttt{auth}  & 5  & 30 s & 3 & \texttt{POST /api/signin}, \texttt{/signup}, \texttt{/refresh} \\
\texttt{clear} & 3  & 10 min & 1 & \texttt{POST /api/clear} \\
...
\texttt{/api/signup}, \texttt{/api/refresh}, and \texttt{/health}.
...
POST & \texttt{/api/signup}        & Register new user            & \texttt{auth} \\
...
POST   & \texttt{/api/save}            & Persist new feature          & \texttt{api} \\
GET    & \texttt{/api/load}            & Paginated feature list        & \texttt{api} \\
PUT    & \texttt{/api/update/:id}      & Update geometry or label      & \texttt{api} \\
DELETE & \texttt{/api/delete/:id}      & Delete feature                & \texttt{api} \\
POST   & \texttt{/api/clear}           & Delete all features           & \texttt{clear} \\
GET    & \texttt{/api/stats}           & Per-type feature counts       & \texttt{api} \\
\midrule
...
GET & \texttt{/api/locations}        & Commune/Daïra/Wilaya lookup  & \texttt{api} \\
GET & \texttt{/api/spatial/commune-boundary} & Commune boundary polygon & \texttt{api} \\
GET & \texttt{/api/scattered-status} & Background task error status & \texttt{api} \\
GET & \texttt{/health}               & Database connectivity probe  & --- \\
\bottomrule
\end{tabularx}
\end{table}
...
The \texttt{/api/load} endpoint uses offset-based pagination:

\begin{equation}
  \texttt{take} = \operatorname{clamp}(\texttt{take}_{\text{req}},\; 1,\; 2{,}000)
  \label{eq:pagination_clamp}
\end{equation}

The response body carries both the data page and the total count (computed
in the same CTE query via a \texttt{COUNT(*)} window), enabling clients to
...
\node[layer, fill=blue!10]   (client)  at (0,0)   {%
  ...
  {\footnotesize Pinia stores \textbar{} useApiFetch composable \textbar{} Snap engine \textbar{} i18n (ar/fr/en)}};
...
\node[layer, fill=red!8]     (db)      at (0,-5.4) {%
  \textbf{Data Layer} \quad PostgreSQL 15 + PostGIS 3 \\
...
ORM / query            & Entity Framework Core + ADO.NET & 9.x \\
Database               & PostgreSQL + PostGIS          & 15 / 3.x \\
...
Draw/edit layer        & Geoman for MapLibre (free)    & 0.7.x \\
...
$d_{\text{geo}}(p, p_0^{(i)}) > \delta_J$ and
$d_{\text{geo}}(p, p_{m_i-1}^{(i)}) > \delta_J$, yet whose nearest point
...
\texttt{POST /api/save}. This allocates a \emph{new} UUID for the
restored feature:
...
\subsection{Minimum Road Length}

A road is rejected on the client if its total metric length is below
the configured minimum:

\begin{equation}
  \ell_{\text{road}} = \sum_{i=0}^{m-2} d_{\text{geo}}(p_i, p_{i+1})
  < L_{\min} = 10\ \mathrm{m}
  \label{eq:min_road_length}
\end{equation}

Length is computed by Turf.js \texttt{length(line, \{units: "meters"\})},
which accumulates the Haversine distance over each segment.

\subsection{Polygon Ring Closure}

The PostGIS \texttt{ST\_GeomFromText} function requires the first and last
vertices of a linear ring to be identical. The client enforces this before
serialisation:

\begin{equation}
  \text{If } p_0 \neq p_{m-1}:\quad \mathcal{C} \leftarrow \mathcal{C} \cup \{p_0\}
  \label{eq:ring_closure}
\end{equation}

where equality is tested coordinate-wise (exact floating-point comparison
on rounded \texttt{float64} values; no epsilon tolerance is needed because
the first vertex is appended verbatim).
...
\node[dec, fill=yellow!20, below=of input] (c1) {Length $\geq L_{\min}$?};
\node[dec, fill=yellow!20, below=of c1] (c2) {Ring closed?};
\node[box, fill=blue!10, below=of c2] (api) {POST /api/validate/\{type\}};
\node[dec, fill=yellow!20, below=of api] (s1) {Angle $\leq 90°$?};
\node[dec, fill=yellow!20, below=of s1] (s2) {Spatial predicate?};
\node[box, fill=green!15, below=of s2] (save) {POST /api/save};
...
\end{tikzpicture}
\caption{Two-stage validation flow for road and district features.
...
The database comprises 14 tables. \Cref{tab:schema} summarises the
...
\texttt{refresh\_tokens}
  ...
  & \texttt{ix\_refresh\_tokens\_token\_hash} \\
\texttt{wilayas}
  & \texttt{wilaya\_id} (int)
  &
...
\texttt{dairas}
  ...
  & \\
\texttt{communes}
  & \texttt{commune\_id} (int)
  & \texttt{daira\_id}
  & \\
\texttt{communes\_boundaries}
  & \texttt{commune\_id} (int)
  &
  & GIST index on \texttt{geometry} \\
\texttt{feature\_registry}
  & \texttt{id} (UUID v7)
  &
  & \texttt{feature\_type} string \\
\texttt{areas}
  & \texttt{id} (UUID v7)
  &
  & \texttt{ix\_areas\_user\_id}, \texttt{ix\_areas\_user\_layer} \\
\texttt{districts}
  & \texttt{id} (UUID v7)
  &
  & \texttt{ix\_districts\_user\_id} \\
\texttt{city\_centers}
  & \texttt{id} (UUID v7)
  &
  & \texttt{ix\_city\_centers\_user\_id} \\
\texttt{roads}
  & \texttt{id} (UUID v7)
  &
  & \texttt{ix\_roads\_user\_id}, \texttt{ix\_roads\_user\_layer} \\
\texttt{house\_entrances}
  & \texttt{id} (UUID v7)
  & \texttt{road\_id} $\to$ \texttt{roads} (nullable)
  & \texttt{ix\_house\_entrances\_road\_id} (partial, \texttt{road\_id IS NOT NULL}) \\
\texttt{public\_buildings}
  & \texttt{id} (UUID v7)
  &
  & \texttt{ix\_public\_buildings\_user\_id} \\
\texttt{public\_spaces}
  & \texttt{id} (UUID v7)
  &
  & \texttt{ix\_public\_spaces\_user\_id} \\
\texttt{naming\_panels}
  & \texttt{id} (UUID v7)
  &
  & \texttt{ix\_naming\_panels\_user\_id} \\
\bottomrule
\end{tabularx}
\end{table}
